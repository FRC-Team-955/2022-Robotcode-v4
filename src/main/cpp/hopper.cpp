...
void Hopper::RunHopperMotor(float hopper_motor_percent_top,float hopper_motor_percent_bottom) {
  talon_hopper_top->Set(ControlMode::PercentOutput,std::max(-hopper_motor_percent_top, -max));
  talon_hopper_bottom->Set(ControlMode::PercentOutput,std::max(-hopper_motor_percent_bottom, -max));
}
